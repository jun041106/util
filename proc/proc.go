...
// Reads through /proc/mounts and returns the data associated devices
func MountPointsByDevice() (map[string][]*MountPoint, error) {
	mp := make(map[string][]*MountPoint)
	var current *MountPoint
	err := ParseSimpleProcFile(
		MountProcFile,
		nil,
		func(line int, index int, elm string) error {
			...
			case 0:
				current = new(MountPoint)
				current.Dev = elm
				if mpSlice, ok := mp[current.Dev]; ok {
					mpSlice = append(mpSlice, current)
				} else {
					mp[current.Dev] = []*MountPoint{current}
				}
			case 1:
				if len(elm) > 0 && elm[0] != '/' {
					return fmt.Errorf("Invalid path on line %d of file %s: %s",
						line, MountProcFile, elm)
				}
				current.Path = elm
			case 2:
